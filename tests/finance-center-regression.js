...
const cashBeforeLegacySeed = snapshot.assetsSummary.cash;
const legacyCashSeed = core.load();
...
malformedDateSeed.ledger.entries.push({ id:"legacy-date-aug-30", type:"expense", date:"115/8/30", amount:100, category:"保險", account:"日期格式測試卡", merchant:"爸爸小額終身壽險" });
...
core.addEntry({ type:"expense", date:today, amount:83, category:"測試", account:"生活帳戶", merchant:"月結後更正收支" });
core.addTransfer({ date:today, amount:83669, fromAccount:"生活帳戶", toAccount:"測試信用卡", note:"月結後更正信用卡繳款" });
let closedCorrectionSnapshot = core.insights();
const closedEntry = closedCorrectionSnapshot.ledger.entries.find(row => row.merchant === "月結後更正收支");
const closedTransfer = closedCorrectionSnapshot.ledger.transfers.find(row => row.note === "月結後更正信用卡繳款");
core.addRecurring({ type:"income", name:"月結後重複固定收支", amount:99, category:"測試", account:"生活帳戶", cycle:"monthly", day:Number(today.slice(8, 10)) });
let duplicateSeed = core.load();
const recurringCorrection = duplicateSeed.ledger.entries.find(row => row.merchant === "月結後重複固定收支");
duplicateSeed.ledger.entries.push({ ...recurringCorrection, id:"closed-month-duplicate-recurring" });
core.persist(duplicateSeed.ledger, duplicateSeed.assets, "seed closed-month recurring duplicate", { backup:false });
core.addCreditBill({ card:"測試信用卡", payAccount:"生活帳戶", billMonth:today.slice(0, 7), amount:83669, dueDate:today });
let duplicateBill = core.insights().ledger.creditBills.find(row => row.amount === 83669);
core.setCreditBillPaid(duplicateBill.id, true);
duplicateSeed = core.load();
const billPayment = duplicateSeed.ledger.transfers.find(row => row.creditBillId === duplicateBill.id);
duplicateSeed.ledger.transfers.push({ ...billPayment, id:"closed-month-duplicate-card-payment", autoGeneratedCreditBillPayment:true, note:`${duplicateBill.billMonth} ${duplicateBill.card} 信用卡帳單繳款` });
core.persist(duplicateSeed.ledger, duplicateSeed.assets, "seed closed-month card payment duplicate", { backup:false });
core.closeMonth(today.slice(0, 7));
assert.equal(core.repairRecurringEntries().removed, 1, "recurring duplicate repair must work after month closeout");
assert.equal(core.repairCreditBillPayments().removed, 1, "card-payment duplicate repair must work after month closeout");
core.removeEntry(closedEntry.id);
core.removeTransfer(closedTransfer.id);
closedCorrectionSnapshot = core.insights();
assert.equal(closedCorrectionSnapshot.ledger.entries.some(row => row.id === closedEntry.id), false, "a deliberate deletion must work after month closeout");
assert.equal(closedCorrectionSnapshot.ledger.transfers.some(row => row.id === closedTransfer.id), false, "a transfer deletion must work after month closeout");
const correctedCloseout = closedCorrectionSnapshot.ledger.monthCloseouts.find(row => row.month === today.slice(0, 7));
assert.equal(correctedCloseout.ledger.entries.some(row => row.id === closedEntry.id), false, "closed-month snapshot must reflect an entry correction");
assert.equal(correctedCloseout.ledger.transfers.some(row => row.id === closedTransfer.id), false, "closed-month snapshot must reflect a transfer correction");
