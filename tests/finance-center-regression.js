...
assert.equal(fs.readFileSync("service-worker.js", "utf8").includes("tsubin-finance-center-v140"), true, "service worker cache must be bumped for finance intelligence upgrades");
assert.equal(financeCenterHtml.includes('FinanceSync.markSynced({remoteUpdatedAt:row.updated_at,comparison:syncState.lastComparison,acknowledgedIds:[]})'), true, "a confirmed cloud version must clear stale review state");
