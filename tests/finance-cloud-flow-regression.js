...
(async()=>{
  ...
  const mobile={ledger:{},assets:{twWatchlist:[{code:'2330',name:'台積電'},{code:'2317',name:'鴻海'}],usWatchlist:[{symbol:'AAPL',name:'Apple'}]}},desktop={ledger:{},assets:{twWatchlist:[{code:'2330',name:'TSMC',updatedAt:'2026-09-22T06:00:00Z'},{code:'2454',name:'聯發科'}],usWatchlist:[{symbol:'MSFT',name:'Microsoft'}]}};const mergedWatch=t.S.mergeBundles(mobile,desktop);assert.deepEqual(mergedWatch.assets.twWatchlist.map(r=>r.code).sort(),['2317','2330','2454']);assert.equal(mergedWatch.assets.twWatchlist.find(r=>r.code==='2330').name,'TSMC');assert.deepEqual(mergedWatch.assets.usWatchlist.map(r=>r.symbol).sort(),['AAPL','MSFT']);
