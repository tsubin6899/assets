(function () {
  ...
  function addCreditBill(values) {
    ...
    upsertCreditStatementCheck(ledger, { card: bill.card, billMonth: bill.billMonth, statementAmount: bill.amount, note: values.note || "", creditBillId: bill.id });
  ...
  function updateCreditBill(id, values) {
    ...
    const next = { card: values.card, billMonth: String(values.billMonth || row.billMonth).slice(0, 7), amount: Math.max(0, number(values.amount)), payAccount: values.payAccount };
    if (row.paid && (next.card !== row.card || next.payAccount !== row.payAccount || next.amount !== number(row.amount))) throw new Error("已繳帳單不可變更卡片、繳款帳戶或金額；請先取消已繳，再修改帳單");
    Object.assign(row, { ...next, dueDate: values.dueDate || "", updatedAt: nowIso() });
    upsertCreditStatementCheck(ledger, { card: row.card, billMonth: row.billMonth, statementAmount: row.amount, note: values.note || "", creditBillId: row.id });
  ...
  function removeCreditBill(id) {
    ...
    ledger.transfers = ledger.transfers.filter(row => {
      if (row.creditBillId !== bill.id) return true;
      if (isGeneratedCreditBillPayment(row)) { recycle(ledger, "transfer", row); return false; }
      row.creditBillId = "";
      return true;
    });
    ledger.creditStatementChecks = ledger.creditStatementChecks.filter(row => row.creditBillId !== bill.id);
    recycle(ledger, "creditBill", bill);
    ledger.creditBills.splice(index, 1);
    return persist(ledger, assets, "刪除信用卡帳單");
  }

  function isGeneratedCreditBillPayment(row) {
    return Boolean(row?.autoGeneratedCreditBillPayment) || /信用卡帳單繳款/.test(String(row?.note || ""));
  }
  ...
  function setCreditBillPaid(id, paid = true) {
    ...
    if (paid) {
      ...
      const preferred = linked.find(row => row.id === bill.transferId) || linked[0] || (ledger.transfers || []).find(row => row.fromAccount === bill.payAccount && row.toAccount === bill.card && number(row.fromAmount) === number(bill.amount) && !row.creditBillId);
      if (preferred) {
        preferred.creditBillId = bill.id;
        preferred.updatedAt = nowIso();
        Object.assign(bill, { paid: true, paidAt: bill.paidAt || nowIso(), transferId: preferred.id, updatedAt:nowIso() });
      } else {
        const transferId = uid("transfer");
        ledger.transfers.push({ id: transferId, date: localDate(), fromAccount: bill.payAccount, toAccount: bill.card, fromAmount: number(bill.amount), toAmount: number(bill.amount), feeAmount: 0, feeAccount: bill.payAccount, creditBillId: bill.id, autoGeneratedCreditBillPayment: true, note: `${bill.billMonth} ${bill.card} 信用卡帳單繳款`, createdAt: nowIso() });
        Object.assign(bill, { paid: true, paidAt: nowIso(), transferId, updatedAt:nowIso() });
      }
    } else if (bill.paid) {
      ledger.transfers = ledger.transfers.filter(row => {
        if (row.creditBillId !== bill.id) return true;
      ...
      Object.assign(bill, { paid: false, paidAt: "", transferId: "", updatedAt:nowIso() });
    }
  ...
  function setCreditBillReconciled(id, reconciled = true) {
    ...
    const bill = ledger.creditBills.find(row => row.id === id);
    if (!bill) throw new Error("找不到這筆信用卡帳單");
    Object.assign(bill, { reconciled: Boolean(reconciled), reconciledAt: reconciled ? nowIso() : "", updatedAt:nowIso() });
  ...
  function saveCreditStatementCheck(values) {
    const { ledger, assets } = load();
    upsertCreditStatementCheck(ledger, values);
